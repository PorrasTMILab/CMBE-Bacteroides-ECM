...
1. ECM substrate term mining — A curated list of ECM‑related substrate terms (provided in Supplementary Table 1) was used to query BRENDA. All enzymes with experimentally reported activity on these substrates were retrieved, along with their associated EC numbers.
2. EC‑based metadata expansion — Each EC number identified from BRENDA was used to pull corresponding UniProtKB entries. These entries supplied additional metadata, including enzyme names, MEROPS identifiers (when available), CAZyme family assignments, and other functional annotations.
3. Cross‑mapping to genome annotations — EC numbers from the ECM database were matched against the EC predictions generated by dbCAN and eggNOG‑mapper, as well as MEROPS family assignments from phmmer searches. This allowed identification of *Bacteroides* genes with predicted ECM‑targeting activity based on concordance between curated ECM enzyme information and genome‑derived annotations.
